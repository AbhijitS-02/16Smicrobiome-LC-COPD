...
create_phyloseq_from_qiime2 <- function(table_file, taxonomy_file, tree_file,
  ...
  if (grepl("\\.qza$", taxonomy_file)) {
    ...
    tax_matrix <- as.matrix(tax_parsed[, c(
      "Kingdom", "Phylum", "Class",
      "Order", "Family", "Genus", "Species"
    )])
    rownames(tax_matrix) <- taxonomy_raw$Feature.ID
  } else {
    tax_df <- read.table(taxonomy_file,
      header = TRUE, sep = "\t",
...
# Define the threshold (1% of total samples)
prev_threshold <- 0.01 * nsamples(ps_silva)

# Keep taxa that have counts > 0 in at least the threshold number of samples
ps_silva_filt <- filter_taxa(
  ps_silva,
  function(x) sum(x > 0) >= prev_threshold,
  TRUE # TRUE prunes the object, FALSE returns a logical vector
)

cat(sprintf("After removing taxa with < 1%% prevalence: %d taxa\n", ntaxa(ps_silva_filt)))
